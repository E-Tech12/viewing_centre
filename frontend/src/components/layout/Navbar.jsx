...
import { Menu, X, Ticket, User, LogOut, ChevronDown, LayoutDashboard } from 'lucide-react'
import { useAuth } from '../../context/AuthContext'

export default function Navbar() {
  const { user, logout, isEventOwner, isPlatformAdmin } = useAuth()
  ...
  // Role-based nav links — staff never see user links
  const navLinks = isEventOwner
    ? [] // owner has sidebar, no top nav
    : isPlatformAdmin
      ? []
      : [
          { to: '/',       label: 'Home'   },
  ...
  const dashboardPath = isEventOwner ? '/owner' : isPlatformAdmin ? '/platform' : null
    ...
    <nav className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-16">
          ...
          <Link to={dashboardPath || '/'} className="flex items-center gap-2 group">
            ...
            {isEventOwner && (
              <span className="hidden sm:flex label-tag bg-ice-400/10 border border-ice-400/20 text-ice-400 text-[9px] ml-1">
                Event Owner
              </span>
            )}
            {isPlatformAdmin && (
              <span className="hidden sm:flex label-tag bg-ember-400/10 border border-ember-400/20 text-ember-400 text-[9px] ml-1">
                Platform Admin
              </span>
            )}
          </Link>
            ...
            {user ? (
              <div className="relative">
                <button onClick={() => setUserMenu(!userMenu)}
                  className="flex items-center gap-2 px-3 py-2 rounded-sm text-slate-300 hover:text-white hover:bg-white/5 transition-all"
                >
                  <div className={`w-7 h-7 rounded-sm flex items-center justify-center border ${
                    isPlatformAdmin ? 'bg-ember-400/20 border-ember-400/30'
                    : isEventOwner  ? 'bg-ice-400/20 border-ice-400/30'
                    :                 'bg-volt-400/20 border-volt-400/30'
                  }`}>
                    <span className={`font-display font-bold text-xs ${
                      isPlatformAdmin ? 'text-ember-400' : isEventOwner ? 'text-ice-400' : 'text-volt-400'
                  ...
                  <div className="absolute right-0 top-full mt-1 w-52 bg-pitch-800 border border-white/10 rounded-sm shadow-2xl overflow-hidden animate-fade-in">
                    {dashboardPath && (
                      <Link to={dashboardPath}
                        className="flex items-center gap-2 px-4 py-3 text-xs text-slate-300 hover:text-white hover:bg-white/5 font-mono uppercase tracking-widest transition-all">
      ...
      {open && (
        <div className="md:hidden bg-pitch-900 border-t border-white/5 px-4 py-4 space-y-1 animate-fade-in">
          {navLinks.map(({ to, label }) => (
            <Link key={to} to={to}
              className="block px-4 py-3 text-sm font-mono uppercase tracking-widest text-slate-400 hover:text-white hover:bg-white/5 rounded-sm">
              {label}
            </Link>
          ))}
          {dashboardPath && (
            <Link to={dashboardPath}
              className="block px-4 py-3 text-sm font-mono uppercase tracking-widest text-volt-400">
              Dashboard
            </Link>
          )}
